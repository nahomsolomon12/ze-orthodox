...
const HomePage = ({ setPage, user }) => {
  const { t } = useLanguage();
  const features = [
    { icon: "play", title: t("featureVideoTitle"), desc: t("featureVideoDesc") },
    { icon: "book", title: t("featureReadingTitle"), desc: t("featureReadingDesc") },
    { icon: "check", title: t("featureAssessTitle"), desc: t("featureAssessDesc") },
    { icon: "trophy", title: t("featureProgressTitle"), desc: t("featureProgressDesc") },
  ];
    ...
    <div>
      <section className="hero">
        ...
        <div className="hero__content">
          <div className="badge badge--gold mb-24">
            <Icon name="church" size={14} /> {t("heroBadge")}
          </div>
          <h1 className="hero__title">
            {t("heroTitleTop")}<br /><span>{t("heroTitleAccent")}</span>
          </h1>
          <Ornament />
      ...
      <section className="section--hero-cta">
        <div className="card card--gold max-w-600" style={{ margin: "0 auto", padding: "40px 32px" }}>
          <h2 className="font-serif mb-0" style={{ fontSize: 24 }}>{t("readyBegin")}</h2>
          <Ornament />
