...
const AuthPage = ({ setPage, setUser }) => {
  ...
  const handleSubmit = () => {
    setErr("");
    if (!form.email || !form.password) return setErr(t("authRequired"));
    if (mode === "signup" && !form.name) return setErr(t("authNameRequired"));

    const user = signIn(form.email, form.name);
    setUser(user);
    setPage("modules");
  };
